...
local status_buf = function() return vim.api.nvim_win_get_buf(vim.g.statusline_winid or 0) end
local icons = { file = "󰈚", lang = "󰈙" }
local devicon = function(kind, key, default_icon)
  local ok, devicons = pcall(require, "nvim-web-devicons")
  if not ok then return default_icon end
  return (kind == "ft" and devicons.get_icon_by_filetype(key, { default = true })) or devicons.get_icon(key, nil, { default = true }) or default_icon
end
...
local format_language = function(bufnr)
  ...
  if ok then lang = parsers.get_buf_lang(bufnr) end
  lang = lang or vim.bo[bufnr].filetype or "Plaintext"
  if lang == "" then return "Plaintext" end
...
local shorten_with_ellipsis = function(path, max_len)
  ...
  for i = #parts, 1, -1 do
    ...
    if used + #part + extra > max_len then break end
...
local format_file_path = function(bufnr)
  ...
  if path == "" then return icons.file, "Empty" end
